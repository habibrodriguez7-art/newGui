...
-- ============================================
-- CREATE DROPDOWN (Sliding Panel from Right)
-- ============================================
function Library:CreateDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue)
    ...
    -- Sliding Panel (inside window, from right side)
    local panelContainer = nil
    local isOpen = false
    
    local function createPanel()
        if panelContainer then panelContainer:Destroy() end
        
        -- Create panel container inside window
        ...
        new("UIPadding", {Parent = searchBox, PaddingLeft = UDim.new(0, 10)})
            ...
            for _, itemName in ipairs(items) do
                if not filter or string.find(itemName:lower(), filter:lower(), 1, true) then
                    ...
                    new("TextLabel", {
                        ...
                        Font = Enum.Font.GothamBold,
                        TextSize = fontSize.small,
                        TextColor3 = selectedItem == itemName and colors.primary or colors.textDim,
                        TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    itemBtn.MouseButton1Click:Connect(function()
                        ...
                        TweenService:Create(panelContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            Position = UDim2.new(1, 0, 0, 42)
                        }):Play()
...
-- ============================================
-- CREATE MULTI DROPDOWN (Sliding Panel from Right)
-- ============================================
            ...
            for _, itemName in ipairs(items) do
                if not filter or string.find(itemName:lower(), filter:lower(), 1, true) then
                    ...
                    local box = new("Frame", {
                        ...
                        BackgroundColor3 = isSelected and colors.primary or colors.bg4,
                        BorderSizePixel = 0,
                        ZIndex = 203
                    })
