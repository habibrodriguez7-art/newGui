...
function Library:CreateWindow(config)
    ...
    -- Tombol minimize: pill/capsule tipis dengan warna primary, posisi kanan header
    ...
    -- Tiga titik horizontal (⋯ style) sebagai ikon minimize
    ...
    for i = 1, 3 do
        local dot = new("Frame", {
            ...
            Position = UDim2.new(0, 6 + (i - 1) * 9, 0.5, -2),
            BackgroundColor3 = colors.primary,
            BackgroundTransparency = 0.2,
            BorderSizePixel = 0,
            ZIndex = 9
    ...
    local function setMinimizeHover(hovering)
        btnMinHeader.BackgroundColor3 = hovering and colors.primary or colors.bg3
        btnMinHeader.BackgroundTransparency = hovering and 0 or 0
        btnMinStroke.Transparency = hovering and 0 or 0.5
        -- Saat hover, dots jadi putih (kontras dengan bg oranye)
        for _, dot in ipairs(dotContainer:GetChildren()) do
            if dot:IsA("Frame") then
                dot.BackgroundColor3 = hovering and colors.text or colors.primary
                dot.BackgroundTransparency = hovering and 0 or 0.2
            end
        end
    end
...
function Library:_switchPage(pageName)
    ...
    for name, data in pairs(self._navButtons) do
        ...
        -- Konsisten dengan warna section/category (bg2 + sectionTransparency)
