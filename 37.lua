...
Library._configTabCreated = false
...
function Library:Cleanup()
    if isDirty then
        local saveOk = pcall(function() Library.ConfigSystem.Save() end)
        if saveOk then
            isDirty = false
        end
    end
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    self:AddConnection(
        "categoryContentSize_" .. tostring(contentContainer),
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    if configPath then
        ...
        RegisterCallback(configPath, onSelect, cbType, cbDef, function(val)
            ...
            if isMulti then
                optionLabel.Text = (type(val) == "table" and #val > 0)
                    and table.concat(val, ", ")
                    or defaultText
            else
                optionLabel.Text = (val ~= nil and tostring(val) ~= "")
                    and tostring(val)
                    or defaultText
            end
        end)
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    local function resolveValue(text)
        if type(text) == "string" then
            local isPureLongInt = #text > 15 and text:match("^%d+$")
            if isPureLongInt then
                return text
            end
        end
        local num = tonumber(text)
        return num ~= nil and num or text
    end
...
function Library:Initialize()
    ...
    if self._pendingWindowObj and not self._configTabCreated then
        self._configTabCreated = true
        pcall(function()
...
function Library:MakeNotify(config)
    ...
    local MAX_NOTIFS = 3
    while #self._activeNotifs >= MAX_NOTIFS do
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddInput(inputConfig)
                ...
                local inputRef = frame and frame:FindFirstChildWhichIsA("Frame")
                inputRef = inputRef and inputRef:FindFirstChildWhichIsA("TextBox")

                return {
                    SetValue = function(self, val)
                        if inputRef and inputRef.Parent then
                            inputRef.Text = tostring(val ~= nil and val or "")
                        end
                    end,
            ...
            function SectionObject:AddParagraph(paragraphConfig)
                ...
                local function reflowParagraph()
                    local totalHeight = 20 -- UIPadding top(10) + bottom(10)
                    ...
                    -- UIListLayout Padding (6px) between title and content
                    if titleLabel and titleLabel.Parent and titleLabel.Visible
                       and contentLabel and contentLabel.Parent and contentLabel.Visible then
        ...
        if self._tabOrder == 1 then
            local nameToSwitch = tabName
            task.defer(function()
                self._library:SetFirstPage(nameToSwitch)
            end)
        end
