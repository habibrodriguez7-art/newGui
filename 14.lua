...
function Library.ConfigSystem.Load()
    -- Auto load disabled, hanya pakai default
...
local function MarkDirty()
    -- Auto save disabled, tidak menyimpan apapun
    return
end
...
-- ============================================
-- GLOBAL BRIDGE (Obfuscated for anti-detection)
-- ============================================
_G._a2s4e = false

function _G._g3c7v(key, default)
    return Library.ConfigSystem.Get(key, default)
end

function _G._s4c8v(key, value)
...
function Library:CreateWindow(config)
    config = config or {}
    local name = config.Name or "aswdasdw"
    local title = config.Title or "L"
...
function Library:Window(config)
    ...
    self:CreateWindow({
        Name = config.Name or ("G_" .. game:GetService("HttpService"):GenerateGUID(false):sub(1,8)),
        Title = config.Title or "L",
