...
function Library:CreateDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue)
    ...
    new("TextLabel", {
        ...
        Position = UDim2.new(0, imageId and 18 or 0, 0, 3),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.small,
        TextColor3 = colors.text,
    ...
    local statusLabel = new("TextLabel", {
        ...
        Position = UDim2.new(0, imageId and 22 or 4, 0, 19),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.small,
    ...
    -- Overlay Container (outside window, positioned dynamically)
    local overlayContainer = nil
    local isOpen = false
    
    local function createOverlay()
        if overlayContainer then overlayContainer:Destroy() end
        
        -- Get window absolute position and size
        local winAbsPos = self._win.AbsolutePosition
        local winAbsSize = self._win.AbsoluteSize
        local headerAbsPos = header.AbsolutePosition
        
        overlayContainer = new("Frame", {
            Parent = self._gui,
            Size = UDim2.new(0, 0, 0, 0),
            Position = UDim2.new(0, headerAbsPos.X, 0, headerAbsPos.Y + 36),
            BackgroundTransparency = 1,
            BorderSizePixel = 0,
            ZIndex = 150,
            ClipsDescendants = false
        ...
        local overlayBg = new("Frame", {
            Parent = overlayContainer,
            Size = UDim2.new(0, dropdownFrame.AbsoluteSize.X, 0, winAbsSize.Y - (headerAbsPos.Y - winAbsPos.Y) - 42),
            BackgroundColor3 = colors.bg2,
            BackgroundTransparency = 0.05,
            BorderSizePixel = 0,
            ZIndex = 151
        })
        new("UICorner", {Parent = overlayBg, CornerRadius = UDim.new(0, 8)})
        ...
        new("UIPadding", {Parent = listContainer, PaddingBottom = UDim.new(0, 3)})
            ...
            for _, itemName in ipairs(items) do
                if not filter or string.find(itemName:lower(), filter:lower(), 1, true) then
                    ...
                    itemBtn.MouseButton1Click:Connect(function()
                        ...
                        task.wait(0.05)
                        if overlayContainer then overlayContainer:Destroy() overlayContainer = nil end
                        isOpen = false
                        arrow.Rotation = 0
...
-- ============================================
-- CREATE MULTI DROPDOWN (Overlay Style)
-- ============================================
