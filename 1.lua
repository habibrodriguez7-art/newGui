-- ============================================
-- LYNX GUI LIBRARY v3.1 - OPTIMIZED COMPACT
-- Modern, Lightweight & Smooth
-- ============================================
...
local colors = {
    ...
    primaryDark = Color3.fromRGB(109, 40, 217),  -- Darker purple
    accent = Color3.fromRGB(59, 130, 246),       -- Blue
    success = Color3.fromRGB(34, 197, 94),       -- Green
    warning = Color3.fromRGB(251, 146, 60),      -- Orange
    error = Color3.fromRGB(239, 68, 68),         -- Red
    
    bg1 = Color3.fromRGB(17, 17, 24),            -- Darkest
    bg2 = Color3.fromRGB(23, 23, 32),            -- Dark
    bg3 = Color3.fromRGB(31, 31, 43),            -- Medium
    bg4 = Color3.fromRGB(41, 41, 56),            -- Light
    
    text = Color3.fromRGB(255, 255, 255),        -- White
    textSub = Color3.fromRGB(203, 213, 225),     -- Light gray
    textMuted = Color3.fromRGB(148, 163, 184),   -- Muted
    
    border = Color3.fromRGB(51, 51, 68),         -- Border
    borderLight = Color3.fromRGB(71, 71, 95),    -- Light border
}

-- Window Config (More compact)
local windowSize = UDim2.new(0, 440, 0, 300)
local minWindowSize = Vector2.new(400, 270)
local maxWindowSize = Vector2.new(650, 500)
local sidebarWidth = 110

-- Animation speeds
local tweenSpeed = 0.2
local tweenSpeedFast = 0.12
...
function Library:CreateWindow(config)
    ...
    local btnMin = new("TextButton", {
        ...
        Position = UDim2.new(1, -33, 0.5, -14),
    ...
    btnMin.MouseEnter:Connect(function()
        tween(btnMin, {BackgroundColor3 = colors.bg4}, tweenSpeedFast)
    end)
    btnMin.MouseLeave:Connect(function()
        tween(btnMin, {BackgroundColor3 = colors.bg3}, tweenSpeedFast)
    end)
    ...
    self._contentBg = new("Frame", {
        Parent = self._win,
        Size = UDim2.new(1, -(sidebarWidth + 8), 1, -46),
        Position = UDim2.new(0, sidebarWidth + 4, 0, 42),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ClipsDescendants = true,
        ZIndex = 11
...
function Library:CreatePage(name, title, imageId, order)
    ...
    btn.MouseLeave:Connect(function()
        if self._currentPage ~= name then
            tween(btn, {BackgroundTransparency = 1}, tweenSpeedFast)
        end
    end)
...
function Library:_switchPage(pageName)
    ...
    for name, data in pairs(self._navButtons) do
        ...
        tween(data.icon, {
            ImageColor3 = isActive and colors.primary or colors.textMuted
        }, tweenSpeedFast)
...
function Library:CreateCategory(parent, title)
    ...
    header.MouseButton1Click:Connect(function()
        ...
        tween(categoryFrame, {BackgroundColor3 = isOpen and colors.bg3 or colors.bg2}, tweenSpeedFast)
    end)
...
function Library:CreateDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue)
    ...
    new("TextLabel", {
        ...
        Position = UDim2.new(0, imageId and 18 or 0, 0, 4),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = 9,
        TextColor3 = colors.text,
    ...
    local statusLabel = new("TextLabel", {
        ...
        Position = UDim2.new(0, imageId and 18 or 0, 0, 18),
        BackgroundTransparency = 1,
        Font = Enum.Font.Gotham,
        TextSize = 8,
        TextColor3 = colors.textMuted,
    ...
    local function createPopup()
        ...
        new("UICorner", {Parent = popupFrame, CornerRadius = UDim.new(0, 6)})
        new("UIStroke", {
            Parent = popupFrame,
            Color = colors.border,
            Thickness = 1
        ...
        local function createItems(filter)
            ...
            for _, itemName in ipairs(items) do
                if not filter or string.find(itemName:lower(), filter:lower(), 1, true) then
                    ...
                    local itemLabel = new("TextLabel", {
                        ...
                        Font = Enum.Font.Gotham,
                        TextSize = 8,
                        TextColor3 = selectedItem == itemName and colors.success or colors.text,
                        TextXAlignment = Enum.TextXAlignment.Left,
                        TextTruncate = Enum.TextTruncate.AtEnd,
                        ZIndex = 304
                    })
                    ...
                    itemBtn.MouseEnter:Connect(function()
                        tween(itemBtn, {BackgroundColor3 = colors.primary}, tweenSpeedFast)
                        tween(itemLabel, {TextColor3 = colors.text}, tweenSpeedFast)
                    end)
                    
                    itemBtn.MouseLeave:Connect(function()
                        tween(itemBtn, {BackgroundColor3 = colors.bg4}, tweenSpeedFast)
                        tween(itemLabel, {
                            TextColor3 = selectedItem == itemName and colors.success or colors.text
                        }, tweenSpeedFast)
                    end)
            ...
            local maxHeight = 150
            local actualHeight = math.min(count * 27, maxHeight)
            listContainer.Size = UDim2.new(1, -8, 0, actualHeight)
            
            -- Animate popup open
            local finalHeight = actualHeight + 36
            tween(popupFrame, {
...
-- ============================================
-- CREATE MULTI DROPDOWN (Popup Style)
-- ============================================
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    new("UICorner", {Parent = inputBg, CornerRadius = UDim.new(0, 5)})
    ...
    inputBox.Focused:Connect(function()
        tween(inputBg, {BackgroundColor3 = colors.primary}, tweenSpeedFast)
    end)
    
    inputBox.FocusLost:Connect(function()
        tween(inputBg, {BackgroundColor3 = colors.bg4}, tweenSpeedFast)
...
function Library:CreateButton(parent, label, callback)
    ...
    button.MouseEnter:Connect(function()
        tween(btnFrame, {BackgroundColor3 = colors.primaryDark}, tweenSpeedFast)
    end)
    
    button.MouseLeave:Connect(function()
        tween(btnFrame, {BackgroundColor3 = colors.primary}, tweenSpeedFast)
    end)
    
    button.MouseButton1Click:Connect(function()
        tween(btnFrame, {Size = UDim2.new(1, 0, 0, 26)}, 0.08)
        task.wait(0.08)
        tween(btnFrame, {Size = UDim2.new(1, 0, 0, 28)}, 0.08)
        pcall(callback)
    end)
...
function Library:CreateTextBox(parent, label, placeholder, configPath, defaultValue, callback)
    ...
    textBox.Focused:Connect(function()
        tween(textBox, {BackgroundColor3 = colors.bg3}, tweenSpeedFast)
    end)
    
    local lastValue = initialValue
    textBox.FocusLost:Connect(function()
        tween(textBox, {BackgroundColor3 = colors.bg4}, tweenSpeedFast)
...
function Library:MakeNotify(config)
    ...
    local finalHeight = content ~= "" and 64 or 44
    tween(notif, {Size = UDim2.new(0, 260, 0, finalHeight)}, tweenSpeed)
