    ...
    local function createMinimizedIcon()
        ...
        local function disconnectIconConns()
            for i = #iconConns, 1, -1 do
                local c = iconConns[i]
                -- FIXED: Add proper validation before disconnect
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    -- FIXED: Debounce thread untuk prevent multiple rapid updates
    local updateThread = nil
    
    -- FIXED: Store arrow rotation tween reference for cleanup
    local _arrowTween = nil

    local function updateSize()
        -- FIXED: Debounce untuk prevent lag saat rapid category toggle
        if updateThread then
            pcall(function() task.cancel(updateThread) end)
        end
        
        updateThread = task.delay(0.03, function()
    ...
    header.MouseButton1Click:Connect(function()
        ...
        -- FIXED: Cancel existing arrow tween before creating new one
        if _arrowTween then
            pcall(function() _arrowTween:Cancel() end)
        end
        
        -- FIXED: Create tween for smooth arrow rotation
        _arrowTween = TweenService:Create(arrow, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
            Rotation = isOpen and 180 or 0
        })
        _arrowTween:Play()
        
        -- FIXED: Direct update saat toggle (user action) untuk responsive feel
        updateSize()
    end)
    
    -- FIXED: Cleanup arrow tween on category destroy
    categoryFrame.Destroying:Connect(function()
        if _arrowTween then
            pcall(function() _arrowTween:Cancel() end)
            _arrowTween = nil
        end
...
function Library:_initDropdownSystem()
    ...
    self:AddConnection("dropdownOverlayClose", closeOverlay.Activated:Connect(function()
        if self._dropdownOverlay.Visible then
            -- FIXED: Cancel panel tween when closing via overlay
            if self._panelTween then
                pcall(function() self._panelTween:Cancel() end)
...
function Library:_showDropdown(dropdownContainer)
    ...
    -- FIXED: Cancel existing panel tween before showing
    if self._panelTween then
        pcall(function() self._panelTween:Cancel() end)
        self._panelTween = nil
    end
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    local function disconnectOptionConns()
        -- FIXED: Optimized connection cleanup untuk prevent lag
        if #optionConns == 0 then return end
    ...
    local function clearOptionFrames()
        ...
        -- FIXED: Optimized frame deletion untuk prevent memory leak
        ...
        if #framesToDelete > 0 then
            -- FIXED: Batch delete dengan yield untuk prevent freeze di low-end devices
            local deleteBatch = 15  -- Optimized untuk low-end devices
            for i, frame in ipairs(framesToDelete) do
                if frame and frame.Parent then
                    -- Priority cleanup: clear attributes, disconnect connections, clear children
                    frame:SetAttribute("RealValue", nil)
                    
                    -- Disconnect any frame-specific connections
                    for _, descendant in ipairs(frame:GetDescendants()) do
                        if descendant:IsA("GuiObject") then
                            -- Clear any event connections by replacing with new instance properties
                            pcall(function()
                                for _, connection in pairs(getconnections and getconnections(descendant.MouseButton1Click) or {}) do
                    ...
                    -- Clear children explicitly before destroy
                    pcall(function()
                ...
                if i % deleteBatch == 0 then
                    task.wait()  -- Yield setiap 15 frames
                end
            end
            
            -- Clear array untuk GC
            table.clear(framesToDelete)
        end
    ...
    local function ensureBuilt()
        ...
        -- FIXED: Reduced batch size untuk low-end devices
        -- Render item dalam batch kecil untuk prevent lag spike
        local batchSize = 15  -- Reduced dari 25 untuk smoother rendering on low-end devices
        for i, opt in ipairs(list) do
            DropdownFunc:AddOption(opt)
            if i % batchSize == 0 then
                task.wait()  -- Yield untuk prevent frame drop
            end
        end
        isBuilt = true
    ...
    function DropdownFunc:SetValues(newList, selecting)
        ...
        -- FIXED: Cancel existing rebuild operation
        if DropdownFunc._isRebuilding then
            DropdownFunc._isRebuilding = false  -- Signal running thread to stop
        end
        
        -- FIXED: Cancel existing rebuild thread
        if DropdownFunc._rebuildThread then
            pcall(function() task.cancel(DropdownFunc._rebuildThread) end)
            DropdownFunc._rebuildThread = nil
        end
        
        clearOptionFrames()
        DropdownFunc.Options = newList
        
        -- Render dalam background thread untuk prevent UI freeze
        DropdownFunc._isRebuilding = true
        DropdownFunc._rebuildThread = task.spawn(function()
            local batchSize = 15  -- Consistent dengan ensureBuilt, optimized untuk low-end devices
            for i, opt in ipairs(newList) do
                if not DropdownFunc._isRebuilding then
                    break  -- Cancel jika ada rebuild baru
                end
                DropdownFunc:AddOption(opt)
                if i % batchSize == 0 then
                    task.wait()
            ...
            DropdownFunc._rebuildThread = nil
            DropdownFunc._isRebuilding = false
        end)
    end
    ...
    self:AddConnection("searchBox_" .. dropdownLayoutOrder, searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        -- FIXED: Increased debounce dari 0.08 ke 0.3 detik untuk reduce lag
...
function Library:CreateButton(parent, label, callback)
    ...
    self:AddConnection("btn_click_" .. label .. tostring(button), button.MouseButton1Click:Connect(function()
        ...
        -- Anti-spam klik cepat (increased dari 0.1 ke 0.3 detik)
...
function Library:MakeNotify(config)
    ...
    -- FIXED: Define notifSpacing in outer scope
    local notifSpacing = 72
    
    -- EXECUTOR FIX: Use separate table for tween tracking instead of Instance property
    local notifTweens = {}
    
    -- PERFORMANCE FIX: Batched repositioning helper function
    local function scheduleReposition()
        if self._repositionScheduled then return end
        self._repositionScheduled = true
        
        task.delay(0.05, function()
            self._repositionScheduled = false
            if not self._activeNotifs then return end
            for i, existingNotif in ipairs(self._activeNotifs) do
                if existingNotif and existingNotif.Parent then
                    local repositionTween = TweenService:Create(existingNotif, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Position = UDim2.new(1, -280, 1, -75 - ((i - 1) * notifSpacing))
                    })
                    -- FIXED: Safely store tween reference (executor-compatible)
                    pcall(function()
                        if existingNotif._tweens then
                            table.insert(existingNotif._tweens, repositionTween)
                        end
                    end)
    ...
    -- FIXED: Stack maksimal 3 notif untuk better visibility, remove oldest jika penuh
    while #self._activeNotifs >= 3 do
        local oldest = self._activeNotifs[1]
        table.remove(self._activeNotifs, 1)
        pcall(function()
            if oldest and oldest.Parent then
                -- Slide out animation sebelum destroy
                local slideOutTween = TweenService:Create(oldest, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                    Position = UDim2.new(1, 20, oldest.Position.Y.Scale, oldest.Position.Y.Offset)
                })
                -- FIXED: Safely store tween reference (executor-compatible)
                pcall(function()
                    if oldest._tweens then
                        table.insert(oldest._tweens, slideOutTween)
                    end
                end)
                slideOutTween:Play()
    ...
    -- FIXED: Optimized reposition - batched update with delayed scheduling
    if #self._activeNotifs > 0 then
        scheduleReposition()
    end
    
    -- Buat notif baru (spawn dari kanan)
    local notifIndex = #self._activeNotifs
    local notif = new("Frame", {
        Parent = self._gui,
        Size = UDim2.new(0, 270, 0, 65),
        Position = UDim2.new(1, 20, 1, -75 - (notifIndex * notifSpacing)), -- Start dari luar layar
        BackgroundColor3 = colors.bg2,
        BackgroundTransparency = panelTransparency,
        BorderSizePixel = 0,
        ZIndex = 200
    ...
    -- EXECUTOR FIX: Store tweens in local table instead of Instance property
    -- notif._tweens = {}  -- Removed: doesn't work reliably in executors
    
    -- Slide in animation
    local slideInTween = TweenService:Create(notif, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, -280, 1, -75 - (notifIndex * notifSpacing))
    })
    table.insert(notifTweens, slideInTween)
    slideInTween:Play()
    ...
    new("UICorner", {Parent = closeBtn, CornerRadius = UDim.new(0, 4)})
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        -- Slide out animation
        local closeSlideOutTween = TweenService:Create(notif, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
            Position = UDim2.new(1, 20, notif.Position.Y.Scale, notif.Position.Y.Offset)
        })
        -- EXECUTOR FIX: Use local table instead of Instance property
        table.insert(notifTweens, closeSlideOutTween)
        closeSlideOutTween:Play()
        ...
        task.delay(0.2, function()
            ...
            -- PERFORMANCE FIX: Use batched repositioning
            scheduleReposition()
        end)
    end)
    new("TextLabel", {
    ...
    -- Auto remove setelah delay dengan smooth animation
    task.delay(delay, function()
        pcall(function()
            if notif and notif.Parent then
                -- Slide out animation
                local autoSlideOutTween = TweenService:Create(notif, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                    Position = UDim2.new(1, 20, notif.Position.Y.Scale, notif.Position.Y.Offset)
                })
                -- EXECUTOR FIX: Use local table
                table.insert(notifTweens, autoSlideOutTween)
                autoSlideOutTween:Play()
    ...
    notif.Destroying:Connect(function()
        -- EXECUTOR FIX: Cancel all tweens from local table
        for i = #notifTweens, 1, -1 do
            local tween = notifTweens[i]
