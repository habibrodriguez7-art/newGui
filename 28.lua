...
function Library:MakeNotify(config)
    ...
    -- TWEEN CLEANUP FIX: Store tween references for proper cleanup
    notif._tweens = {}
    ...
    table.insert(notif._tweens, slideInTween)
    slideInTween:Play()
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        -- FIXED: Safely access _tweens table (executor-compatible)
        pcall(function()
            if notif._tweens then
                table.insert(notif._tweens, closeSlideOutTween)
            end
        end)
    ...
    task.delay(delay, function()
        pcall(function()
            if notif and notif.Parent then
                ...
                pcall(function()
                    if notif._tweens then
                        table.insert(notif._tweens, autoSlideOutTween)
                    end
                end)
    ...
    notif.Destroying:Connect(function()
        -- TWEEN CLEANUP FIX: Cancel all active tweens before destroy (executor-compatible)
        pcall(function()
            if notif._tweens then
                for i = #notif._tweens, 1, -1 do
                    local tween = notif._tweens[i]
