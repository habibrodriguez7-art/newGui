...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    local function clearOptionFrames()
        ...
        if #framesToDelete > 0 then
            ...
            local deleteBatch = 20  -- Reduced untuk device low-end
            for i, frame in ipairs(framesToDelete) do
                if frame and frame.Parent then
                    -- Clear attributes dan connections sebelum destroy
                    frame:SetAttribute("RealValue", nil)
                    frame:Destroy()
                end
                if i % deleteBatch == 0 then
                    task.wait()  -- Yield setiap 20 frames
                end
            end
    ...
    local function ensureBuilt()
        ...
        local batchSize = 25  -- Reduced dari 80 untuk smoother rendering
    ...
    function DropdownFunc:SetValues(newList, selecting)
        ...
        -- FIXED: Set flag untuk prevent multiple rebuilds
        ...
        DropdownFunc._rebuildThread = task.spawn(function()
            local batchSize = 25  -- Consistent dengan ensureBuilt
    ...
    self:AddConnection("searchBox_" .. dropdownLayoutOrder, searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        -- FIXED: Increased debounce dari 0.08 ke 0.2 detik untuk reduce lag
        searchThread = task.delay(0.2, function()
            searchThread = nil
            for _, entry in ipairs(optionFrameCache) do
...
function Library:MakeNotify(config)
    ...
    while #self._activeNotifs >= 3 do
        ...
        pcall(function()
            if oldest and oldest.Parent then
                ...
                TweenService:Create(oldest, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
    ...
    -- FIXED: Optimized reposition - only reposition if there are existing notifications
    local notifSpacing = 72 -- 65 height + 7 spacing
    if #self._activeNotifs > 0 then
        for i, existingNotif in ipairs(self._activeNotifs) do
            if existingNotif and existingNotif.Parent then
                local targetY = 1
                local targetYOffset = -75 - ((i - 1) * notifSpacing)
                TweenService:Create(existingNotif, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Position = UDim2.new(1, -280, targetY, targetYOffset)
                }):Play()
            end
        end
    end
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        task.delay(0.2, function()
            ...
            -- Reposition semua notif yang masih ada
            if self._activeNotifs then
                local notifSpacing = 72
                for i, existingNotif in ipairs(self._activeNotifs) do
                    if existingNotif and existingNotif.Parent then
                        TweenService:Create(existingNotif, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
