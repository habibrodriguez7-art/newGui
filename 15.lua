...
function Library:_switchPage(pageName)
    ...
    for name, data in pairs(self._navButtons) do
        ...
        -- FIX: active pakai bg3 (lebih terang), inactive pakai bg2 transparan
        data.btn.BackgroundColor3 = isActive and colors.bg3 or colors.bg2
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    -- FIX: pakai Instance pointer sebagai key unik, bukan tostring(frame) yang bisa sama
    local categoryKey = "categoryToggle_" .. tostring(header)
    self:AddConnection(categoryKey, header.MouseButton1Click:Connect(function()
        isOpen = not isOpen
...
function Library:MakeNotify(config)
    ...
    -- FIX: stack notif, bukan replace. Hapus hanya jika sudah > 5 notif aktif
    local MAX_NOTIFS = 5
    while #self._activeNotifs >= MAX_NOTIFS do
        local old = table.remove(self._activeNotifs, 1)
        pcall(function()
            if old and old.Parent then old:Destroy() end
        end)
    end

    -- Geser notif yang ada ke atas
    for i, existingNotif in ipairs(self._activeNotifs) do
        if existingNotif and existingNotif.Parent then
            local offsetY = -75 - (i * 70)
            existingNotif.Position = UDim2.new(1, -280, 1, offsetY)
        end
    end
